...
func Padded(size uint, dst, src []byte) []byte {
	/*
...
func B58EncodeCheck(ver uint8, b []byte) (s string) {
	/* Prepend version */
...
// 生成密钥对
func MakeNewKey(randSeed string) (*SayKey, error) {
	var err error
	var sayKey SayKey
	...
	private, err := ecdsa.GenerateKey(curve, strings.NewReader(randSeed))
	if err != nil {
		log.Panic(err)
	}
